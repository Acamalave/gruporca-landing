    ...
    <section id="contacto" className="py-20 bg-white">
      <div ref={ref} className={`max-w-7xl mx-auto px-4 sm:px-6 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        <div className="grid lg:grid-cols-2 gap-12">
          <div>
            <span className="text-brand-gold font-semibold text-sm uppercase tracking-widest">Cotizacion rapida</span>
            <h2 className="text-3xl sm:text-4xl font-black text-brand-navy mt-2 mb-3">Solicita tu cotizacion</h2>
            ...
            {status === "sent" ? (
              <div className="bg-green-50 border border-green-200 rounded-2xl p-8 text-center">
                ...
                <h3 className="text-xl font-bold text-green-700 mb-2">Cotizacion enviada</h3>
                <p className="text-green-600 text-sm">Nuestro equipo se pondra en contacto contigo a la brevedad.</p>
                <button onClick={() => setStatus("idle")} className="mt-4 text-green-700 underline text-sm">Enviar otra cotizacion</button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-brand-navy mb-1">Que necesitas? *</label>
                ...
                {status === "error" && <p className="text-red-500 text-sm text-center">Error al enviar. Intenta de nuevo o contactanos por WhatsApp.</p>}
              </form>
            )}
          </div>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl overflow-hidden h-64 lg:h-80">
              <iframe
                ...
                width="100%" height="100%" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="Ubicacion Grupo RCA"
              />
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              {[
                { icon: "M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z", title: "Direccion", lines: ["Autopista Valencia - Campo Carabobo", "Via de Servicio San Luis, Valencia"] },
                { icon: "M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z", title: "Telefono", lines: ["(0241) 700-6000", "info@gruporca.com"] },
